...
{
	Student s;
	cout << "\nmEDIE ESTE: " << s.getMedie();
	s.setMedie(10);
	cout << "\nmedia dupa set este " << s.getMedie();

	cout << "\ntipul bursei este " << s.getBursa();
	s.setBursa(SOCIALA);
	cout << "\ntipul bursei dupa set este " << s.getBursa();

	cout << "\nverifica daca are bursa: " << s.getareBursa();
	s.setareBursa(true);
	cout << "\nverificare bursa dupa set " << s.getareBursa();
	return 0;
}
